(function () {
  ...
  function loadProgress() {
    try {
      ...
      return raw ? JSON.parse(raw) : { completedCases: {} };
    } catch (e) {
  ...
  function renderMasthead() {
    ...
    if (state.view === "citySelect" || state.view === "caseSelect") {
      titles.innerHTML =
        '<p class="kicker">Le Stéphanois — Dossiers d\'enquête</p><h1>' +
        (state.view === "caseSelect" ? currentCity().name : state.data.gameTitle) +
        "</h1>";
    } else {
    ...
    if (state.view === "citySelect") {
      ...
      el.appendChild(pts);
    } else if (state.started && state.view !== "caseSelect") {
