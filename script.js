(function () {
  ...
  function renderCitySelect() {
    ...
    state.data.cities.forEach((city) => {
      ...
      } else if (city.status === "coming_next") {
        card.className = "city-card locked coming-next";
        card.innerHTML =
          '<span class="city-name">' +
          city.name +
          '</span><span class="city-status next-tag">🔜 Prochaine enquête</span>';
        card.disabled = true;
      } else {
        card.innerHTML =
          '<span class="city-name">' +
  ...
  function renderFootnote() {
    ...
    el.textContent =
      "Enquête fictive, personnages et faits imaginaires.";
    return el;
  }

  // ---------------------------------------------------------------
  // Suggestion de rotation (mobile portrait). On ne peut pas fiabiliser
  // un vrai verrouillage d'orientation (Safari iOS ne le permet pas
  // pour une page web classique), donc on propose plutôt un message
  // que le joueur peut ignorer, mémorisé pour la session en cours.
  // ---------------------------------------------------------------
  function initOrientationHint() {
    try {
      if (sessionStorage.getItem("orientationHintDismissed") === "1") {
        document.body.classList.add("orientation-hint-dismissed");
      }
    } catch (e) {
      // ignore
    }
    const dismissBtn = document.getElementById("orientation-hint-dismiss");
    if (dismissBtn) {
